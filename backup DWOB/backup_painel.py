...
try:
    ...
    print(f"⚠️ módulos não encontrados: {e}")
    ...
    while True:
        ...
        try:
            ...
            if t.is_alive():
                ...
                print(f"⏱️ IQ TIMEOUT: {call_name} {duration:.1f}s")
            elif res[1] is not None:
                result_box["error"] = res[1]
                print(f"❌ IQ ERROR: {call_name} {duration:.1f}s -> {res[1]}")
            else:
                result_box["value"] = res[0]
                if duration >= 0.5:
                    print(f"⏳ IQ SLOW: {call_name} {duration:.1f}s")
        except Exception as e:
            result_box["error"] = e
        finally:
            result_event.set()
...
def atualizar_hora():
    """Hora precisa com interpolação IQ."""
    global lbl_hora
    try:
        agora = _get_server_now()
...
def salvar_historico(texto):
    try:
        ...
        if log:
            janela.after(0, escrever_log, log, f"ERRO HISTORICO: {e}", "info")
...
def montar_painel():
    ...
    def atualizar_candle_real():
        ...
        if candle_thread_rodando:
            canvas.after(1000, atualizar_candle_real)  # 🔥 MUITO IMPORTANTE
            return
        ...
        # 🔁 LOOP SEMPRE CONTINUA (NUNCA PARA)
